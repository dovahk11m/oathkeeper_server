...
public class PlanScenario01_Completed {
        ...
        public void create() {
                ...
                Plan completedPlan = Plan.builder().creatorMember(user1).group(sampleGroup)
                                .title("주말 코딩 스터디").planDatetime(LocalDateTime.now().minusDays(3))
                                .status(Status.COMPLETED).placeName(seomyeon.getName())
                ...
                Participant p1 = createParticipant(completedPlan, user1,
                                completedPlan.getPlanDatetime().minusMinutes(10),
                                MovementStatus.ARRIVED, false);
                Participant p2 = createParticipant(completedPlan, user2,
                                completedPlan.getPlanDatetime().plusMinutes(5),
                                MovementStatus.ARRIVED, false);
                Participant p3 = createParticipant(completedPlan, user3,
                                completedPlan.getPlanDatetime(), MovementStatus.ARRIVED, false);
                Participant p4 = createParticipant(completedPlan, user4,
                                completedPlan.getPlanDatetime().plusMinutes(15),
                                MovementStatus.ARRIVED, false);
                ...
                // 6. Group 통계 업데이트
                // ========================================
                // 완료된 Plan의 통계를 Group에 누적
                sampleGroup.addPlanStatistics(completedPlan);

                // 7. Group AI 요약 설정
                String groupSummary = "분석된 1개의 약속에 따르면, 이 그룹은 약속당 평균 7.1km를 이동했으며, "
                                + "평균 5분의 지각 시간을 기록했습니다. 총 이동 시간은 152분(약 2시간 30분)이었습니다. "
                                + "참가자 4명 중 2명이 정시에 도착하여 50%의 정시 도착률을 보였습니다. "
                                + "전반적으로 적절한 거리를 이동했으며, 다음 약속에서는 조금 더 여유롭게 출발하면 "
                                + "모두가 편안하게 만날 수 있을 것입니다.";
                sampleGroup.updateSummary(groupSummary,
                                com.oath.domain.groups.SummaryStatus.COMPLETED);

                // 8. Group 저장
                groupRepository.save(sampleGroup);

                log.info("👷‍♂️ [Scenario 1] '완료된 약속' 샘플 데이터 생성 완료 (Plan ID: {}, Group ID: {})",
                                completedPlan.getId(), sampleGroup.getId());
                log.info("📊 [Scenario 1] Group 통계 업데이트: 완료된 약속 {}개, 총 거리 {}km, 총 시간 {}분",
                                sampleGroup.getTotalPlansCompleted(),
                                sampleGroup.getTotalTravelDistance(),
                                sampleGroup.getTotalTravelTime());
        }
