...
public class PlanScenario02_Ongoing {
    ...
    public void create() {
        ...
        Plan ongoingPlan =
                Plan.builder().creatorMember(user1).group(sampleGroup).title("긴급 트러블슈팅 회의")
                        .planDatetime(LocalDateTime.now().plusHours(1)).status(Status.CONFIRMED)
        ...
        Participant p1 = createParticipant(ongoingPlan, user1, null, MovementStatus.MOVING, true);
        Participant p2 = createParticipant(ongoingPlan, user2,
                ongoingPlan.getPlanDatetime().minusMinutes(5), MovementStatus.ARRIVED, false);
        ...
        Participant p4 = createParticipant(ongoingPlan, user4,
                ongoingPlan.getPlanDatetime().plusMinutes(2), MovementStatus.ARRIVED, false);
        ...
        log.info(
                "👷‍♂️ [Scenario 2] '진행중인 약속' 데이터 생성 완료 (Plan ID: {}). user1(participantId:{}) 도착 시 COMPLETED 상태로 변경됩니다.",
