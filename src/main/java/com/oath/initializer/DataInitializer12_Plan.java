 ...
 * <p>
 * 이 클래스는 애플리케이션 시작 시(local 프로필) 약속(Plan) 관련 샘플 데이터를 생성하는 역할을 총괄합니다.
 * 실제 데이터 생성 로직은 각각의 '시나리오(Scenario)' 클래스에 위임하고, 여기서는 어떤 시나리오를 어떤 순서로 실행할지만 결정합니다.
 * </p>
 *
 * <h3>새로운 Plan 시나리오를 추가하는 방법:</h3>
 * <ol>
 *     <li>`com.oath.initializer.scenario` 패키지 내에 새로운 시나리오 클래스를 생성합니다. (예: `PlanScenario04_Cancelled.java`)</li>
