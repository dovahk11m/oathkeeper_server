...
AI 요약 보고서 생성은 비동기로 처리되며, 완료까지 시간이 소요될 수 있습니다. 클라이언트는 `202 Accepted` 응답을 받으면, 일정 시간 간격을 두고 동일한 API를 재호출하여 최종 결과를 확인해야 합니다.

-   **권장 폴링 간격**: `3 ~ 5초`
-   **권장 타임아웃**: 총 `30초` (예: 5초 간격으로 6번 폴링 후에도 `202 Accepted`가 반환되면 실패 처리)
-   **재시도 로직**: `500 Internal Server Error`가 반환되거나, 타임아웃 발생 시 사용자에게 알림 후 재시도 옵션을 제공하는 것을 권장합니다.
